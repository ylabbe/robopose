...
def loss_refiner_CO_disentangled_reference_point(TCO_possible_gt,
    ...
    TCO_pred_orn[:, :3, :3] = pose_update_with_reference_point(TCO_input, K_crop,
                                                               torch.cat((vxvy_gt, vz_gt), dim=-1),
                                                               dR, tCR)[:, :3, :3]

    # Second term: influence of vxvy
    TCO_pred_xy = TCO_gt.clone()
    TCO_pred_xy[:, :2, [3]] = pose_update_with_reference_point(TCO_input, K_crop,
                                                               torch.cat((vxvy, vz_gt), dim=-1),
                                                               dR_gt, tCR)[:, :2, [3]]

    # Third term: influence of vz
    TCO_pred_z = TCO_gt.clone()
    TCO_pred_z[:, [2], [3]] = pose_update_with_reference_point(TCO_input, K_crop,
