...
class Plotter:
    ...
    def load_logs(self, run_ids):
        ...
        for run_id, color in zip(run_ids, self.colors_hex):
            ...
            config = yaml.full_load(cfg_path.read_text())
            ...
            configs[run_id].delta_t = f'{(datetime.datetime.now() - last_write).seconds / 60:.1f}' + f'({len(log_df)})'
    ...
    def show_configs(self, ignore=None, diff=True):
        if ignore is None:
            ignore = ('n_workers', 'save_dir', 'job_dir', 'seed',
                      ...
                      'test_only_last_epoch', 'hardware', 'run_comment')
        ignore = list(ignore)
        ...
        if 'possible_anchors' in df.columns:
            df['possible_anchors'] = df['possible_anchors'].apply(str)
        if 'possible_anchor_links' in df.columns:
            df['possible_anchor_links'] = df['possible_anchor_links'].apply(str)
        if diff:
