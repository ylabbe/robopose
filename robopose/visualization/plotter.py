...
class Plotter:
    ...
    def make_overlay(self, rgb_input, rgb_rendered):
    ...
    def plot_overlay(self, rgb_input, rgb_rendered, figure=None):
        rgb_input = np.asarray(rgb_input)
        rgb_rendered = np.asarray(rgb_rendered)
        assert rgb_input.dtype == np.uint8 and rgb_rendered.dtype == np.uint8
        mask = ~(rgb_rendered.sum(axis=-1) == 0)
        ...
        f = self.plot_image(overlay, name='image', figure=figure)
        return f
